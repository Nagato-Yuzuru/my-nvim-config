...
function M.root_dir(markers)
	return function(bufnr, on_dir)
		local r = M.resolve(bufnr, markers)
		if r == nil then
			return
		end
		if r == false then
...
-- Servers that hand-roll root_dir for mutual-exclusion / multi-marker logic
-- the helper can't express. Central layer keeps hands off these.
--   * denols / vtsls: deno vs node mutual exclusion (whoever's marker is deeper wins)
--   * eslint: defers to denols when buffer is inside a deno project
...
--
-- 设计取舍：强制覆盖比"只在 root_dir 缺失时注入"更鲁棒——避免 lsp/*.lua 里
-- 复制粘贴的 fallback 兜底重新引入扫 $HOME 的 bug。需要保留自定义 root_dir
-- 的 server 显式加入 SKIP 即可（成本：3 行，加入时点是写自定义 root_dir 的
-- 同一动作，难以遗漏）。
---@param names string[]
	...
	for _, name in ipairs(names) do
		if not SKIP[name] then
			...
			if cfg and cfg.root_markers then
				local patch = { root_dir = M.root_dir(cfg.root_markers) }
