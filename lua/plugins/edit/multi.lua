...
return {
	{
		...
		config = function()
			...
			set({ "n", "x" }, "<A-n>", function() mc.matchAddCursor(1) end, { desc = "MC: Select Next" })
			-- <A-S-n> SelectAllOccurrences
			set({ "n", "x" }, "<A-S-n>", mc.matchAllAddCursors, { desc = "MC: Select All" })
			-- <A-x>   FindNextOccurrence（跳过当前，找下一个）
			set({ "n", "x" }, "<A-x>", function() mc.matchSkipCursor(1) end, { desc = "MC: Skip & Next" })
			-- <A-p>   UnselectPreviousOccurrence（移除当前光标）
			set({ "n", "x" }, "<A-p>", mc.deleteCursor, { desc = "MC: Remove Cursor" })

			-- ── 行模式：逐行添加光标 ──────────────────────────────────
			set({ "n", "x" }, "<A-j>", function() mc.lineAddCursor(1) end, { desc = "MC: Add Cursor Down" })
			set({ "n", "x" }, "<A-k>", function() mc.lineAddCursor(-1) end, { desc = "MC: Add Cursor Up" })
